...
- `session-5` branch off `main`, holding the rate-limiting and backup work plus a new archived OpenSpec change; not yet merged to `main`.
- 9 capability specs (all synced), no active OpenSpec changes.
- Backend restarted on this branch's code and live-verified for the rate limiter; not otherwise redeployed.
