...
Intent routing:
- Document questions ΓåÆ retrieval_tool
- Analysis/summary ΓåÆ analyst_tool
- External info ΓåÆ web_search_tool
...
**Tasks**:
- [ ] Create integration tests for pipeline (PDF ΓåÆ Vector DB)
...
def main():
    ...
    print(f"Γ£à GitHub token found")
    print(f"≡ƒôª Repository: {GITHUB_OWNER}/{GITHUB_REPO}")
    print(f"≡ƒôï Creating {len(ISSUES)} issues...")
    print()
    ...
    for i, issue_data in enumerate(ISSUES, 1):
        try:
            ...
            print(f"    Γ£à Created: #{result['number']} - {result['html_url']}")
    ...
    print("Summary")
    print("=" * 70)
    print(f"Γ£à Successfully created: {len(created_issues)} issues")
    print(f"Γ¥î Failed: {len(failed_issues)} issues")
